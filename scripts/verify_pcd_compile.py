    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            for table, preset in (
                ...
                if not row:
                    print(f"ERROR: preset instance manquant: {preset} ({table})")
                    return 1

            delays = {
                r[0]
                for r in conn.execute(
                    "SELECT name FROM delay_profiles WHERE name LIKE 'FR-Delay-%'"
                ).fetchall()
            }
            if delays != {"FR-Delay-Radarr", "FR-Delay-Sonarr"}:
                print(f"ERROR: delay profiles attendus FR-Delay-Radarr/Sonarr, got {delays}")
                return 1
