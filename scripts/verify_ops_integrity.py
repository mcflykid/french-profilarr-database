...
def main() -> int:
    ...
    pat_04_early = set(
        re.findall(
            r"re\.name = '([^']+)'",
            read("04-custom-format-conditions.sql"),
        )
    )
    orphan_regex = sorted(regex_names - pat_04_early)
    if orphan_regex:
        warnings.append(f"02: regex sans liaison 04: {orphan_regex}")
    ...
    cf_05 = set(
        re.findall(
            r"INSERT(?: OR IGNORE)? INTO custom_format_tags \(custom_format_name, tag_name\)\s+SELECT '([^']+)'",
            t05,
        )
    )
    ...
    radarr_naming_09 = set(RE_RADARR_NAMING_SELECT.findall(t09))
    delays_09 = [n for n in RE_DELAY_NAME.findall(t09) if n.startswith("FR-Delay-")]
    if radarr_media_09 != set(RADARR_PROFILES):
        fail("09-profile-media-bundles.sql", f"radarr media: {radarr_media_09 ^ set(RADARR_PROFILES)}")
    elif sonarr_media_09 != set(SONARR_PROFILES):
        fail("09-profile-media-bundles.sql", f"sonarr media: {sonarr_media_09 ^ set(SONARR_PROFILES)}")
    elif radarr_naming_09 != set(RADARR_PROFILES):
        fail("09-profile-media-bundles.sql", "radarr naming incomplet")
    ...
    print(f"\n09-profile-media-bundles.sql\n  radarr: {len(radarr_media_09)}, sonarr: {len(sonarr_media_09)}")
    ...
    # --- 11 / 12 tests (optionnels — conteneur parser Profilarr) ---
    for optional in ("11-custom-format-tests.sql", "12-quality-profile-tests.sql"):
        path = OPS / optional
        if not path.is_file():
            print(f"\n{optional}\n  (absent — optionnel)")
            continue
        if optional.startswith("11"):
            t11 = path.read_text(encoding="utf-8")
            tests = [
            ...
            if dup_tests:
                fail(optional, f"{len(dup_tests)} doublons (cf,title,type)")
            elif bad_cf_test:
                fail(optional, f"CF inconnus: {sorted(bad_cf_test)[:5]}")
            else:
                ok(optional)
            print(f"\n{optional}\n  {len(tests)} tests, doublons: {len(dup_tests)}, CF invalides: {len(bad_cf_test)}")
        else:
            t12 = path.read_text(encoding="utf-8")
            entities = RE_ENTITY.findall(t12)
            dup_ent = [k for k, c in Counter((a, int(b)) for a, b in entities).items() if c > 1]
            if dup_ent:
                fail(optional, f"test_entities dupliqués: {dup_ent}")
            else:
                ok(optional)
