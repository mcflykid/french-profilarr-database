...
Pour un usage simple, utilise **Customisations** dans Profilarr (pas de second dépôt requis).
...
| `10-profile-ui-tags.sql` | Réservé vide — tags UI profils dans `06` + `01` |
| `11` / `12` | Tests parser |

Modification de `ops/09-profile-media-bundles.sql` : éditer le SQL puis `python3 scripts/validate.py`.
...
Utiliser la branche **`main`** avec `pcd.json` version **1.1.0** (schema 1.1.0).
